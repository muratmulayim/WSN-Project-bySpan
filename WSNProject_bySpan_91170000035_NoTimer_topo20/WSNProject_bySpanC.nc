...
implementation {
	...
	*/
	...
	event void Boot.booted() {	
		...
		uint8_t i;
		uint8_t size = sizeofArray(topology20[ indexOf(TOS_NODE_ID) ]);
		
		spans[ indexOf(TOS_NODE_ID) ] = size;
		
		for(i = 0; i < size; i++){
			neighborNodes[ i ] = topology20[ indexOf(TOS_NODE_ID) ][ i ];
		}
		...
		for(i = 0; i < size; i++){
			...
			spans[ indexOf(neighborNodes[ i ]) ] = sizeofArray(topology20[ indexOf(neighborNodes[ i ]) ]);
